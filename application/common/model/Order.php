...
class Order extends BaseModel
{
     ...
     */
    public function getAllReceivedOrderId($user_id,$day){
    	// 截止时间
    	...
    	$orderIds = $this->where($filter)->column('order_id');
//     	// 将已发货的订单日期取出跟指定日期一一对比，超过了的修改状态
...
//     	], ['order_id' => ['in', $orderIds]]);
    ...
    private function updatePayStatus($transaction_id)
    {
        ...
        try {
            // 更新商品库存、销量
            $ItemModel = new Item;
            $order_id = $this->getFieldByWhere(['order_no'=>$transaction_id], 'order_id');
            $order_info = Db::name('order_goods')
            						->where(['order_id'=>$order_id])
            						->select();
            $ItemModel->updateStockSales($order_info);
            // 更新订单状态
            ...
            // 升级
            if ($this['item_id'] == 10 && $this['pay_price'] == 99){
            	$user->upUserLevel($this['user_id'], time());
            }
            // 更新prepay_id记录
    ...
    {
        $status = [10 => '进行中', 20 => '取消', 30 => '已完成', 40=>'退款'];
        return ['text' => $status[$value], 'value' => $value];
    }
    /**
     * 生成订单号
    ...
    */
     ...
     * @throws \think\exception\DbException
     */
    public function getListsSupplier($dataType,$query = [],$user_id)
    {
     ...
     */
    public function returnbuy($order_id,$user_id){
    ...
    public function getFieldByWhere($where,$filed){
    	if ($where){
    		$data = $this->where($where)->value($filed);
    		if ($data){
    			return $data;
    		}
    	}
     ...
     * @return unknown|boolean
     */
    public function countReceiveData($where){
    	if ($where){
    		$data = $this->where($where)->sum('pay_price');
	    	if ($data){
	    		return $data;
	    	}
    	}
     ...
     * @return unknown|boolean
     */
    public function countData($where, $type){
    	if ($where && $type){
    		if ($type == 'success'){
    			$filter = [
    					'pay_status' => 20,// 已支付
    		...
    		$data = $this->where($where)->where($filter)->count();
    		if ($data){
    			return $data;
    		}
    	}
