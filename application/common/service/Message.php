...
use app\common\model\Settingagnet as DealerSettingModel;
...
class Message
{
     ...
     */
    public function delivery($order,$express,$express_no)
    {
        // 获取 prepay_id
        $prepayId = $this->getPrepayId($order['order_id']);
        // 微信模板消息
    ...
    {
        // 模板消息id
        $template = DealerSettingModel::getItem('template_msg', $withdraw['app_id']);
    ...
    {
        // 模板消息id
        $template = DealerSettingModel::getItem('template_msg', $dealer['app_id']);
        ...
        $status = $this->sendTemplateMessage($dealer['app_id'], [
            'touser' => $user['open_id'],
    ...
    {
        $str = '';
        foreach ($goodsData as $Item) $str .= $Item['name'] . ' ';
        return $str;
    }
