...
{
    /* @var \app\common\model\Goods $Item */
    private $Item;
    private $user_id;
    ...
    public function getImage()
    {
        ...
        // 下载商品首图
        $goodsUrl = $this->saveTempImage($appId, $this->goods['image'][0]['file_path'], 'item');
        // 下载小程序码
        $scene = "gid:{$this->goods['item_id']},uid:" . ($this->user_id ?: '');
        $qrcode = $this->saveQrcode($appId, $scene, 'pages/goods/index');
        // 拼接海报图
    ...
    private function savePoster($backdrop, $goodsUrl, $qrcode)
    {
        ...
        // 商品名称处理换行
        $fontSize = 30;
        $goodsName = $this->wrapText($fontSize, 0, $fontPath, $this->goods['name'], 680, 2);
        // 写入商品名称
    ...
    private function getPosterPath()
    {
        ...
        // 保存路径
        $tempPath = WEB_PATH . 'temp' . '/' . $this->goods['app_id'] . '/';
    ...
    private function getPosterName()
    {
        ...
        return 'goods_' . md5("{$this->goods['item_id']}_{$this->user_id}") . '.png';
    }
    ...
    {
        return $this->posterUrl ?: (\base_url() . 'temp/' . $this->goods['app_id'] . '/' . $this->getPosterName());
    }
